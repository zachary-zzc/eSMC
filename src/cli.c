...
//	fprintf(stderr, "         -I FLOAT    amplitude of random initialization for EM iteration [%lg]\n", p->ran_init);
    ...
    fprintf(stderr, "         -A FLOAT    initial admixture time; -1 to disable [-1]\n");
...
psmc_par_t *psmc_parse_cli(int argc, char *argv[])
{
	...
	psmc_par_t *par;
	par = psmc_new_par();
	while ((c = getopt(argc, argv, "i:t:l:r:N:p:o:dI:c:bSDT:A:")) >= 0) {
		switch (c) {
        ...
        case 'A': par->at0 = atof(optarg); if (par->at0 >= 0) par->flag |= PSMC_F_ADMIX; break;
